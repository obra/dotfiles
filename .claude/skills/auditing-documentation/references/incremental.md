...
1. Worklist: `scripts/docmaint stale`. Reasons mean:
...
2. Also run `scripts/docmaint scan` — terminology never depends on stamps,
...
4. Stamping precondition (hub law): at least one independent verifier
   ...
   `scripts/docmaint stamp --set <doc> [--deferred N]`.
